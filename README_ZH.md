...
Dashboard 会写入或刷新协议相关文件，并触发接口脚手架生成。常见协议源位置是：
...
默认使用 Docker Hub 的 `ros:<distro>-ros-base`，例如 `ros:humble-ros-base`。
如果你的网络环境需要私有 registry 镜像缓存，可以显式配置 Harbor：
...
通用视觉依赖可通过 `ENABLE_VISION_STACK=1` 启用：
...
`infra/communication` 目前只实现了部分通信中间件矩阵。module `config.yaml`
里建议只使用这些高层 route middleware：

| Middleware | 状态 |
| --- | --- |
| `nats` | 已实现。 |
| `ros2` | 已实现。 |
| `cyclonedds` | 已实现，但按数据路径部分支持。 |
| `fastdds` | 已实现，但按数据路径部分支持。 |
| `zenoh`、`grpc`、`mqtt` | 仅预留，尚未实现。 |

具体语言覆盖和限制见 [infra/communication/README.md](./infra/communication/README.md)。
