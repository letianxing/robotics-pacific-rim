...
import {
	InputGroup,
	InputGroupAddon,
} from "@dashboard/ui/components/input-group";
...
function CommandInput({
	...
	return (
		<div className="border-b" data-slot="command-input-wrapper">
			<InputGroup className="h-11 rounded-none border-none bg-transparent shadow-none">
				<InputGroupAddon>
...
function CommandList({
	...
	return (
		<CommandPrimitive.List
			className={cn(
				"max-h-80 scroll-py-2 overflow-y-auto overflow-x-hidden p-1 outline-none",
				className
			)}
...
function CommandItem({
	...
	return (
		<CommandPrimitive.Item
			className={cn(
				"group/command-item relative flex cursor-default select-none items-center gap-2 rounded-sm px-2 py-2 text-sm outline-hidden data-[disabled=true]:pointer-events-none data-selected:bg-accent data-selected:text-accent-foreground data-[disabled=true]:opacity-50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0 data-selected:*:[svg]:text-accent-foreground",
				className
			)}
...
function CommandShortcut({
	...
	return (
		<span
			className={cn(
				"ml-auto text-muted-foreground text-xs group-data-selected/command-item:text-accent-foreground",
				className
			)}
