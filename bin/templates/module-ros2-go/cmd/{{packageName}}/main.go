...
import (
	...
	generatedapi "github.com/pacific-rim/pacific-rim/module/service/{{name}}/internal/api/generated"
	"github.com/pacific-rim/pacific-rim/module/service/{{name}}/internal/service"
)
...
func main() {
	...
	runtime, err := commbootstrap.BootstrapCommunication(ctx, *configPath, "{{name}}")
	if err != nil {
	...
	if err := app.Run(ctx); err != nil && !errors.Is(err, context.Canceled) {
		log.Fatalf("run {{packageName}}: %v", err)
	}
}
