...
`{{name}}` is a ROS2 Go module. It owns API handlers, service use cases, and
...
- `cmd/{{packageName}}/main.go`: process entry, signal handling, observability
...
- The communication runtime expands only the user-facing middleware family
  ...
  config.
- Go ROS2 native support is a build/deployment concern behind the same route
  config. The template runs `tools/generate-ros2-bindings.sh` from CMake when
  `PACIFIC_RIM_GO_BUILD_TAGS=pacific_rim_ros2_rclgo` is set, then imports the
  generated package through `internal/api/generated`; deployments may use a
  sidecar bridge without changing business route config. Do not add rclgo
  imports to business code.
...
`pkg/idl/<service>/generated/go`. Provider routes add provider slots,
route-specific `ports`, byte-level wrappers, and middleware registrars.
Consumer routes add current-service `client.go` or `subscriber.go` protocol
...
`internal/service/generated/service.go`, and user-editable implementations
under `internal/service/generated`. Provider routes implement public ports from
`pkg/idl/<service>/generated/go/ports.go`; consumer routes delegate through
current-service generated client/subscriber adapters. Do not edit pkg generated
role files.
`cmd/{{packageName}}/main.go` already calls the
module-local generated registration hook. Infra bootstrap owns middleware
client/backend registration and fabric connection; generated Go code only binds
configured routes to module business implementations. Existing service
implementations are not overwritten by ordinary generation; use `--force` only
to intentionally reset them. Scaffold generation reads provider routes from
public pkg interfaces and consumer routes from local `config.yaml`; local config
must not duplicate provider routes.
...
`Execute(ctx, endpoint, payload)` style publisher method. Consumer routes also
create module-local fill points without copying provider IDL:
`direction: subscribe` creates a subscriber callback, and `direction: client`
creates a downstream client helper. Keep `topic_ref` and `service_ref` in
...
- "If this uses CycloneDDS request/reply, should `standard` be `omg_dds_rpc`
  or `rmw_cyclonedds`?"
- "If this Go service needs native CycloneDDS, should I enable the
  `pacific_rim_cyclonedds` build tag and Docker runtime dependency?"
