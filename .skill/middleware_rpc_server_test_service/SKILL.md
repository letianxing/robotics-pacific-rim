# Middleware Rpc Server Test Development Skill

Use this skill whenever an agent or engineer changes `module/service/middleware_rpc_server_test_service`.
...
`middleware_rpc_server_test_service` is a ROS2 Go module. It owns API handlers, service use cases, and
...
- `cmd/middleware_rpc_server_test/main.go`: process entry, signal handling, observability
...
- This skill lives at `.skill/middleware_rpc_server_test_service/SKILL.md`; do
...
- Ownership matrix:
  - Provider/publisher routes owned by this module go in
    `pkg/idl/middleware_rpc_server_test_service/public/interfaces.yaml`.
...
consumer routes delegate through current-service pkg protocol role files. Do not edit pkg generated role files. `cmd/middleware_rpc_server_test/main.go` already
calls the module-local generated registration hook. Infra bootstrap owns
middleware client/backend registration and fabric connection; generated Go code
only binds configured routes to module handlers. Existing service
